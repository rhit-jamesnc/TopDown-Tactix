...
describe('calculateCpuImpulse - Branch Coverage', () => {
  const mockPlayer = { position: { x: 100, y: 100 } } as Matter.Body;
  const mockBall = { position: { x: 15, y: 100 } } as Matter.Body;
  beforeEach(() => resetCpuState());

  it('should trigger sweet spot logic when distToTarget < 50', () => {
      const impulse = calculateCpuImpulse(mockPlayer, mockBall, 800, 600, 'first-team');
      expect(impulse.x).not.toBe(0);
  });
});