...
export const OnlineGameCanvas = () => {
  ...
  useEffect(() => {
    ...
    const createVisual = (id: string, color: string, radius: number) => {
      const body = Matter.Bodies.circle(0, 0, radius, { isStatic: true });
    ...
    createVisual('p1', '#ef4444', 25);
    createVisual('p2', '#3b82f6', 25);
    ...
    socket.on('game-state', (state: GameState) => {
      ...
      if (state.players) {
        Object.entries(state.players).forEach(([id, data]) => {
          if (visualBodies[id]) Matter.Body.setPosition(visualBodies[id], data.position);
        });
      }
    });
